...
namespace Robot.Builders.Extensions
{
    ...
    public class SymmetryTool : MonoBehaviour
    {
        ...
        private Transform _toMirror;
        private Transform _mirrored;
    
        private readonly Dictionary<Transform, Transform> _mirrorPairs = new Dictionary<Transform, Transform>();
    
        private int _lastToMirrorChildCount;
        private Vector3 _lastToMirrorPos;
        private Quaternion _lastToMirrorRot;
        private Vector3 _lastToMirrorScale;
        private MirrorPlane _lastPlane;
        ...
        void SetupChildren()
        {
            ...
            if (existingToMirror == null)
            {
                ...
                _toMirror = toMirrorObj.transform;
            }
            else
            {
                _toMirror = existingToMirror;
            }
            ...
            if (existingMirrored == null)
            {
                ...
                _mirrored = mirroredObj.transform;
            }
            else
            {
                _mirrored = existingMirrored;
            }
        ...
        void InitializeLastValues()
        {
            if (_toMirror != null)
            {
                _lastToMirrorChildCount = _toMirror.childCount;
                _lastToMirrorPos = _toMirror.localPosition;
                _lastToMirrorRot = _toMirror.localRotation;
                _lastToMirrorScale = _toMirror.localScale;
            }
            _lastPlane = plane;
        }
        ...
        void OnEditorUpdate()
        {
            if (!autoUpdate || _toMirror == null || _mirrored == null) return;
        
            _updateFrameCounter++;
        
            // Check transforms every frame
            bool needsUpdate = (_toMirror.childCount != _lastToMirrorChildCount ||
                                _toMirror.localPosition != _lastToMirrorPos ||
                                _toMirror.localRotation != _lastToMirrorRot ||
                                _toMirror.localScale != _lastToMirrorScale ||
                                plane != _lastPlane);
            ...
            else if (syncComponents && _updateFrameCounter % 5 == 0)
            {
                SyncComponentValues();
            }
        ...
        {
            return CheckTransformChangesRecursive(_toMirror);
        }
        ...
        {
            if (_mirrorPairs.Count == 0) return;
        ...
        {
            if (_toMirror == null || _mirrored == null)
            {
                SetupChildren();
                return;
            }
            ...
            while (_mirrored.childCount > 0)
            {
                DestroyImmediate(_mirrored.GetChild(0).gameObject);
            }
            ...
            Vector3 mirroredPos = MirrorPosition(_toMirror.localPosition, planeNormal);
            _mirrored.localPosition = mirroredPos;
        
            Quaternion mirroredRot = MirrorRotation(_toMirror.localRotation, planeNormal);
            _mirrored.localRotation = mirroredRot;
        
            _mirrored.localScale = _toMirror.localScale;
        
            // Duplicate all children
            foreach (Transform child in _toMirror)
            {
                DuplicateAndMirror(child, _mirrored, planeNormal);
            }
        }
        ...
        void DuplicateAndMirror(Transform original, Transform parent, Vector3 planeNormal)
        {
            ...
            _mirrorPairs[original] = dupTransform;
