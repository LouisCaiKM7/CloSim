...
namespace Online.Replay.Recorder
{
    ...
    public sealed class MatchReplayRecorderRunner : MonoBehaviour
    {
        ...
        {
            // Constructed directly (mirrors how Online.Rooms.LobbyServices lazily constructs
            // MasterServerClient) so this stays decoupled from any particular DI/singleton wiring.
            // Config is blank (ReplayServiceConfig) until the user supplies an AWS endpoint — IsConfigured
            // is false today, so this degrades to a no-op log line and never blocks or throws.
            var service = new Online.Replay.Service.ReplayServiceClient();
            if (!service.IsConfigured)
            {
                Debug.Log("[MatchReplayRecorderRunner] Replay service not configured (blank endpoint) — recording discarded, not uploaded.");
                return;
            }
            ...
            {
                Debug.LogWarning($"[MatchReplayRecorderRunner] Replay upload threw unexpectedly: {e.Message}");
                return;
            }

            if (!result.ok)
                Debug.LogWarning($"[MatchReplayRecorderRunner] Replay upload failed: {result.error}");
            else
                Debug.Log($"[MatchReplayRecorderRunner] Replay uploaded: {result.replayId}");
        }
