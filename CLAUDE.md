...
- **Replay feature (new — additive to the original 4-phase plan) — merged end-to-end.** Deterministic **state-snapshot** match replays (not video), all under `Online.Contracts.Replay` / `Online.Replay.*`:
  ...
  - **Recorder** (`Online.Replay.Recorder.ReplayRecorder` + `MatchReplayRecorderRunner`): host-only, hooked into `MatchSceneBootstrap`'s existing online branch (no-ops on clients and offline play); samples every robot's position/rotation at `ReplayFormat.DefaultTickRate`, keyframes every `DefaultKeyframeInterval` frames, and uploads via `ReplayServiceClient` on match end. Game-piece snapshots are deferred — see the file's header comment — so replays are robots-only for now.
  - **In-game Replays list + playback** (`Online.Replay.UI.ReplaysScreen`, `ReplayPlaybackScreen`, `ReplayRowUI`, `ReplaysMenuController`, reachable from the main menu's "Replays" button): lists stored replays natively (never a web page — same reasoning that keeps the room system as "Server List", not "Server Browser"), and plays one back by loading its recorded field scene and driving locally-spawned, non-networked robot representations from the decoded timeline. v1 simplifications: robots only (no pieces), a single fixed third-person camera, and keyframe-granular stepping instead of a full scrub bar — see that file's header comment for the complete list.
  - **AWS S3 config (bucket/region/credentials) stays blank** until the user supplies it, per golden rule 2 — replay upload/download is a graceful no-op until then, and the game is unaffected either way.
